...
export const createBooking = async (bookingData) => {
  try {
    // Generate a custom booking ID (unless one is already provided)
    const id = bookingData.id || `REQ-${1000 + Math.floor(Math.random() * 9000)}`;
    ...
    // Special handling for admin-created bookings that should be automatically approved
    let isAdminCreated = bookingData.isAdminCreated || bookingData.adminCreated;
    
    // Log the important status values we're working with
    console.log('Creating booking with status info:', { 
    ...
    const newBooking = {
      id,
      ...bookingData,
      // For admin-created bookings, always set status to approved
      // Otherwise, use the provided status or default to pending_faculty
      status: isAdminCreated ? 'approved' : (bookingData.status || 'pending_faculty'),
      // For admin-created bookings, always set both approvals to 'approved'
      // Otherwise, use the provided approval status or default
      approvalStatus: isAdminCreated ? {
        faculty: 'approved',
        admin: 'approved'
    ...
    // Special handling for admin-created bookings that are marked as approved
    let finalBookingData = { ...newBooking };
    
    // Properly handle all date fields to ensure they're strings, not Timestamp objects
    const processDateFields = (obj) => {
      // Convert specific known date fields to strings if they're objects
      const dateFields = ['submittedAt', 'lastModified', 'date', 'preferredDate', 'actualDate', 'requestDate'];
      ...
      dateFields.forEach(field => {
        if (field in processed) {
          // Convert Firebase Timestamp objects
          if (processed[field] && typeof processed[field] === 'object' && processed[field].toDate) {
            processed[field] = processed[field].toDate().toISOString();
          } 
          // Handle actual JavaScript Date objects
          else if (processed[field] instanceof Date) {
            processed[field] = processed[field].toISOString();
          }
          // Ensure we don't have any objects that can't be serialized
          else if (processed[field] && typeof processed[field] === 'object' && !Array.isArray(processed[field])) {
            processed[field] = String(processed[field]);
          }
        }
      });
      
      // Handle special case for actualTimeSlot
      if (processed.actualTimeSlot) {
        if (typeof processed.actualTimeSlot === 'object') {
          // Convert start and end times if they're Date objects
          if (processed.actualTimeSlot.start instanceof Date) {
            processed.actualTimeSlot.start = format(processed.actualTimeSlot.start, 'HH:mm');
          } else if (processed.actualTimeSlot.start && typeof processed.actualTimeSlot.start === 'object' && processed.actualTimeSlot.start.toDate) {
            processed.actualTimeSlot.start = format(processed.actualTimeSlot.start.toDate(), 'HH:mm');
          }
          
          if (processed.actualTimeSlot.end instanceof Date) {
            processed.actualTimeSlot.end = format(processed.actualTimeSlot.end, 'HH:mm');
          } else if (processed.actualTimeSlot.end && typeof processed.actualTimeSlot.end === 'object' && processed.actualTimeSlot.end.toDate) {
            processed.actualTimeSlot.end = format(processed.actualTimeSlot.end.toDate(), 'HH:mm');
          }
        }
      }
    ...
    finalBookingData = processDateFields(finalBookingData);
...
const directClientSideUpdate = async (docId, newStatus, feedback, updatedBy, additionalData = null) => {
  try {
    ...
    if (additionalData) {
      ...
      // IMPORTANT: Add specific properties explicitly to avoid them getting lost
      ...
      // IMPORTANT: Add the approval status explicitly
      if (additionalData.approvalStatus) {
        updatedBooking.approvalStatus = additionalData.approvalStatus;
      }
      
      // Add approval notes if present
      if (additionalData.approvalNotes) {
        updatedBooking.approvalNotes = additionalData.approvalNotes;
      }
      
      // Add the explicitly set status
      if (additionalData.status) {
        updatedBooking.status = additionalData.status;
      }
      
      // Only add other properties after the critical ones are set directly
      // This prevents properties from being overwritten with generic assignment
      Object.keys(additionalData).forEach(key => {
        if (!['actualDate', 'actualTimeRange', 'approvalStatus', 'approvalNotes', 'status'].includes(key)) {
          updatedBooking[key] = additionalData[key];
        }
      });
    }
    ...
    console.log('Final updated booking object (client-side):', updatedBooking);
